...
registerBlockType('ea-gaming-engine/arcade', {
			...
			<div {...useBlockProps()}>
				...
				<label>
					...
					<input
						...
						value={courseId || ''}
						onChange={(e) =>
							setAttributes({ courseId: parseInt(e.target.value, 10) })
						}
					/>
				</label>
				<label>
					...
					<select
						value={theme}
						onChange={(e) => setAttributes({ theme: e.target.value })}
					>
						<option value="playful">{__('Playful', 'ea-gaming-engine')}</option>
						<option value="minimal_pro">
							{__('Minimal Pro', 'ea-gaming-engine')}
						</option>
					</select>
				</label>
				<label>
					...
					<select
						value={preset}
						onChange={(e) => setAttributes({ preset: e.target.value })}
					>
	...
	save: ({ attributes }) => {
		...
		return (
			<div
				className="ea-gaming-launcher"
				data-course-id={courseId}
				data-theme={theme}
				data-preset={preset}
			>
				<button className="ea-gaming-start">
					{__('Start Game', 'ea-gaming-engine')}
				</button>
			</div>
		);
	}
